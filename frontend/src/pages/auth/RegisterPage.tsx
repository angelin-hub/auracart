...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Welcome to Yehovah Boutique!");
      navigate("/");
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 pt-16 pb-8" style={{ background: BG }}>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-72 h-72 rounded-full blur-[120px] opacity-18"
          style={{ background: BLUSH }} />
        <div className="absolute bottom-1/4 right-1/3 w-48 h-48 rounded-full blur-[90px] opacity-12"
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm" style={{ color: "rgba(44,35,32,0.5)" }}>
            Discover exclusive women's fashion
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            {[
              ...
              <div key={f.id}>
                ...
                <input
                  id={f.id} type={f.type} value={form[f.key]}
                  onChange={set(f.key)}
            ...
            {/* Password */}
            <div>
              <label htmlFor="reg-pass" className="block text-sm font-medium mb-1.5"
                style={{ color: "rgba(44,35,32,0.7)" }}>Password</label>
                  ...
                  <div className="flex-1 flex gap-1">
                    {[1,2,3,4].map(i => (
                      <div key={i} className="h-1 flex-1 rounded-full transition-all"
                        style={{ background: i <= strength ? strengthColor : "rgba(44,35,32,0.1)" }} />
                    ))}
                  </div>
